...
| Metric | n | Prior mean (raw) |
|---|---|---|
| Batting Average | 334 | 37.401 |
| Runs per Player-Match | 334 | 58.964 |
| Century Rate | 334 | 0.128 |
| Bowling Average | 248 | 32.053 |
| Wickets per Bowler-Match | 248 | 3.283 |
| Five-Wicket-Haul Rate | 248 | 0.148 |
| Ten-Wicket-Match Rate | 248 | 0.022 |

## Representative player scores

Roles shown are the declared/evaluation roles. `raw → adjusted` shows the shrinkage effect.
...
| Jack Hobbs | opener | 61 | 56.9→52.1 | 88.7→81.3 | 165.0→132.2 | 0.016→0.823 | **96.4** |
| Len Hutton | opener | 79 | 56.7→52.8 | 88.2→82.3 | 77.3→68.2 | 0.038→0.693 | **97.2** |
| Sachin Tendulkar | middle-order | 200 | 53.8→52.3 | 79.6→77.7 | 54.2→52.2 | 0.230→0.508 | **96.6** |
| Steve Smith | middle-order | 125 | 56.0→53.4 | 87.4→83.4 | 53.0→50.1 | 0.152→0.584 | **99.0** |
| Kumar Sangakkara | middle-order | 134 | 57.4→54.8 | 92.5→88.2 | n/a→n/a | 0.000→0.426 | **99.2** |
| Brian Lara | middle-order | 131 | 52.9→50.8 | 91.2→87.0 | n/a→n/a | 0.000→0.435 | **97.6** |
| Viv Richards | middle-order | 121 | 50.2→48.4 | 70.6→68.9 | 61.4→57.2 | 0.264→0.693 | **86.6** |
| Ricky Ponting | middle-order | 168 | 51.9→50.4 | 79.6→77.4 | n/a→n/a | 0.030→0.376 | **95.3** |
| Innocent Kaia | opener | 3 | 62.4→40.7 | 104.0→64.8 | n/a→n/a | 0.000→2.855 | **68.5** |
...
| Glenn McGrath | fast-bowler | 124 | 7.4→11.6 | 5.2→12.6 | 21.6→23.1 | 4.540→4.366 | **86.5** |
| Malcolm Marshall | fast-bowler | 81 | 18.9→22.6 | 22.3→29.6 | 20.9→23.1 | 4.642→4.373 | **93.8** |
| Donald Tiripano | fast-bowler | 16 | 22.1→30.6 | 33.2→47.5 | 49.0→39.6 | 1.625→2.546 | **18.1** |

## Hot-streak check (the reason for shrinkage)

- Innocent Kaia (3 Tests): **68.5**
- Devdutt Padikkal (4 Tests): **80.4**
- Sachin Tendulkar (200 Tests): **96.6**

No player with ≤5 Tests appears in any role's top 10 (top-10 minimums: 12–87 Tests).
...
### Shape 1 — 2O / 4MO / 1K / 1AR / 3F (elite)

- Don Bradman (opener): 100.0
- Len Hutton (opener): 97.2
- Kumar Sangakkara (middle-order): 99.2
...
```bash
npm test   # 236 assertions (127 XI + 120-trial draft sim + 107 seven-metric)
# regenerate this file:
node_modules/.bin/esbuild scripts/review-report.ts --bundle --platform=node --format=cjs --outfile=.diag-dist/r.cjs --log-level=error && node .diag-dist/r.cjs
```

`/matchup` is still on the old `ratings.ts` model — nothing here is live until you approve wiring it up.
