...
def extract_era_data(content, era_name):
    ...
    print(f"Looking for '{start_pattern}' at index {start_idx}")
    if start_idx == -1:
    ...
    print(f"Found closing parenthesis at index {end_paren_idx}")
    if end_paren_idx == -1:
    ...
    print(f"Content after era marker first 100 chars: {repr(content_after_era[:100])}")

    # Find the next era section (## ) or end of content
    next_era_idx = content_after_era.find('## ')
    print(f"Next '## ' found at index {next_era_idx}")
    if next_era_idx == -1:
        section = content_after_era
        print("Taking section to end of content")
    else:
        section = content_after_era[:next_era_idx]
        print(f"Taking section up to next era, length {len(section)}")

    print(f"Section content first 200 chars: {repr(section[:200])}")

    # Split by nation sections (lines starting with ### )
    # First, split on newline followed by ###
    nation_sections = re.split(r'\n### ', section)
    print(f"Split into {len(nation_sections)} nation sections")

    for i, ns in enumerate(nation_sections[:3]):  # Show first 3
        print(f"Nation section {i} first 100 chars: {repr(ns[:100])}")
    ...
    for nation_section in nation_sections:
        ...
        print(f"Processing nation header: {repr(nation_header)}")
        # Remove the leading "### "
        ...
        table_lines = lines[1:]
        print(f"Table has {len(table_lines)} lines")
        if table_lines:
            print(f"First table line: {repr(table_lines[0])}")

        players = parse_markdown_table(table_lines)
        print(f"Parsed {len(players)} players from table")
