...
import { buildPopulations, rawMetrics, percentileRank, BOWLING_METRICS, evaluationRole } from '../src/lib/seven-metrics';
...
for (const id of ['jacques-kallis','garfield-sobers','imran-khan']) {
  const p = byId.get(id)!; const raw = rawMetrics(p);
  ...
  for (const k of BOWLING_METRICS) {
    ...
    console.log(`  ${k}: raw=${v} below=${bl} equal=${eq} n=${n} -> ${pct.toFixed(1)} (ordinal ~${Math.round(bl + eq/2)} of ${n})`);
  }
}
